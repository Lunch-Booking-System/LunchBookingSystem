...
const VendorNavbar = () => {
  ...
  const handleSnacksNavigation = () => {
    ...
    const { sessionId } = JSON.parse(session);
    const vendorId = sessionId;
    router.push(`/snacks/${vendorId}`);
  };

  const handleBreakfastNavigation = () => {
    const session = localStorage.getItem("vendorSession");
    if (!session) {
      toast.error("You are not logged-in");
      router.push("/onboardingvendor/login");
    ...
    const { sessionId } = JSON.parse(session);
    const vendorId = sessionId;
    router.push(`/breakfast/${vendorId}`);
  };

  const handleLunchDinnerNav = () => {
    const session = localStorage.getItem("vendorSession");
    if (!session) {
      toast.error("You are not logged-in");
      router.push("/onboardingvendor/login");
    ...
    const { sessionId } = JSON.parse(session);
    const vendorId = sessionId;
    router.push(`/Lunch/Dinner/${vendorId}`);
  };
  ...
  return (
    <div className="flex justify-between gap-x-5 shadow-xl pb-2">
      ...
      <div className="flex mx-5">
        {/* <p
          className="text-lg mt-8 mr-4 md:mr-10 cursor-pointer hover:scale-105 duration-150 hover:font-bold"
        ...
        <p
          className="text-lg mt-8 mr-4 md:mr-10 cursor-pointer hover:scale-105 duration-150 hover:font-bold"
          onClick={handleSnacksNavigation}
        >
          Snacks
        </p>
        <p
          className="text-lg mt-8 mr-4 md:mr-10 cursor-pointer hover:scale-105 duration-150 hover:font-bold"
          onClick={handleBreakfastNavigation}
        >
          Breakfast
        </p>
        <p
          className="text-lg mt-8 mr-4 md:mr-10 cursor-pointer hover:scale-105 duration-150 hover:font-bold"
          onClick={handleLunchDinnerNav}
        >
          Lunch/Dinner
        </p>
        <p
          className="text-lg mt-8 mr-4  md:mr-10 cursor-pointer hover:scale-105 hover:font-bold duration-150"
